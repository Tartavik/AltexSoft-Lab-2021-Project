(()=> {
  ...
  function createPaginator(length) {
    ...
    const newUrl = `_start=${quantityPagination * limitOfItems - 10}&_limit=50`;
  ...
  function showPagination() {
    const newUrl = `_start=${paginations.length * limitOfItems - this.name}&_limit=50`;
    getUsersInfo(newUrl);
      ...
      for (let i = 0; i < inputFilter.length; i++) {
          inputFilter[i].addEventListener('keyup',e => {
              if (e.keyCode >= 48 && e.keyCode <= 90 || e.keyCode === 8) {
                  let resultSort = '';
                  const newUrl = `_start=${paginations.length * limitOfItems - document.querySelector('.active').name}&_limit=50`;
  ...
  function getUsersInfo(arg) {
    const urlParams = arg? `?${arg}`:'';
    sendRequsest('GET', requestUrl + urlParams)
