...
buttonStart.addEventListener("click", () => {
  let appropriateNumbers = findAppropriateNumbers(
    dominoUp.value + dominoDown.value
  );
  let arrAppropriateDoableNumbers =
    сheckOnQuantityAppropriateNumbers(appropriateNumbers);
  if (arrAppropriateDoableNumbers.length > 0) {
    ...
    alert("It is impossible to sort this sequence");
  }
});
...
function rotateSomeNumbers(arr) {
  ...
  if (quantityСhange > arrDominoUp.length / 2) {
    ...
    arrNumberLineDominoСhange = arrAllNumbers.filter(
      (el) => !arrNumberLineDominoСhange.includes(el)
    );
...
function changeDomino(el) {
  ...
  console.log(
    arrDominoUp[0],
    el.currentTarget.id,
    arrDominoUp[+el.currentTarget.id - 1],
    arrDominoUp
  );
